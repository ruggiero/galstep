...
def phi(double rho, double z, double Mf, double Rdf, double z0f):
	global M, Rd, z0
	M = Mf
	Rd = Rdf
	z0 = z0f
	...
	sum_ = 0.0
	for l in [0, 2, 4, 6, 8]:
		t1 = (1/r**(l+1)) * quad(phi_integrand_1, 0, r, args=(l), full_output=1)[0]
		t2 = r**l * quad(phi_integrand_2, r, np.inf, args=(l), full_output=1)[0]
		sum_ += Pl(cos(theta), l) * (t1 + t2)
	return -np.pi*G * sum_


cdef double rho_l(double r, int l):
	return quad(rho_l_integrand, 0, np.pi, args=(r, l), full_output=1)[0]
...
def rho_l_integrand(double theta, double r, int l):
	...
	rho = r * sin(theta)
	z = r * cos(theta)
	return M / (4 * np.pi * z0 * Rd**2) * (1 / cosh(z / (2*z0))**2) * exp(-rho/Rd) * sin(theta) * Pl(cos(theta), l)
